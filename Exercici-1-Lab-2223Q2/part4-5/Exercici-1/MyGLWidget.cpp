...
{
  glm::mat4 TG(1.0f);
  TG = glm::rotate(TG,angle2,glm::vec3(0.f,0.f,1.f)); 
  TG = glm::translate(TG,posicio*scl);  
  TG = glm::scale(TG,glm::vec3(0.25*scl)); 
...
void MyGLWidget::modelTransformCavall(glm::vec3 posicio)
{
  ...
  TG = glm::scale(TG,glm::vec3(0.08*scl)); 
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_C: 
      if(menja) { 
        posblanc = glm::vec3(0.125,0.125,0.0);;
        menja = false;
      }
      else {    
        posblanc = glm::vec3(-0.125,-0.375,0.0);
        menja = true;
      }
    	break;
