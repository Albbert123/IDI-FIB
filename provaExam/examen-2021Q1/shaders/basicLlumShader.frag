...
void main()
{	
    vec3 fnormalSCO2 = normalize(normalSCO);

    vec4 posFocus1 = vec4(posFocus,1); //foco camera

    vec3 L = normalize((posFocus1 - vertexSCO).xyz);

    vec3 difus = Difus(fnormalSCO2, L, colFocus);
    vec3 ambient = Ambient();

    vec3 Lambert = ambient + difus;

    vec3 phong = Especular(fnormalSCO2, L, vertexSCO, colFocus);
