...
void MyGLWidget::paintGL () 
{
    ...
    glViewport (0, 0, ample*(float(newAmple)/100.f), alt*(float(newAltura)/100.f));
    //glViewport (0, 0, ample, altura);
    ...
    if(modelPatricio) {
        ...
        modelTransformP (); 
        glDrawArrays(GL_TRIANGLES, 0, nVerticesP);
        ...
        legomanTransform();
        glDrawArrays(GL_TRIANGLES, 0, nVerticesL);
    }
...
void MyGLWidget::creaBuffersLegoman() {
    l.load("/home/albert/Documentos/UPC/Q4/IDI/Bloc2/s4/ej2/Model/legoman.obj");

    caja_legoman();

    nVerticesL = l.faces().size()*3;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*nVerticesL*3, l.VBO_vertices(), GL_STATIC_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*nVerticesL*3, l.VBO_matdiff(), GL_STATIC_DRAW);
...
void MyGLWidget::creaBuffersPatricio() 
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*nVerticesP*3, m.VBO_vertices(), GL_STATIC_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*nVerticesP*3, m.VBO_matdiff(), GL_STATIC_DRAW);
...
void MyGLWidget::legomanTransform() {
  // Matriu de transformació de model
  glm::mat4 transform (1.0f);
  transform = glm::rotate(transform, angP, glm::vec3(0.0, 1.0, 0.0));
  transform = glm::scale(transform, glm::vec3(escala)); 
  transform = glm::scale(transform, glm::vec3(1/alturaL, 1/alturaL, 1/alturaL)); //altura 1
  transform = glm::translate(transform, -centreLego);
  glUniformMatrix4fv(transLoc, 1, GL_FALSE, &transform[0][0]);
}
...
void MyGLWidget::caja_legoman() {
    xmax = xmin = float(l.vertices()[0]);
    ymax = ymin = float(l.vertices()[1]);
    zmax = zmin = float(l.vertices()[2]);

    for (unsigned int i = 0; i < l.vertices().size(); i+=3) {
        
        xmax = std::max(xmax,float(l.vertices()[i]));
        xmin = std::min(xmin,float(m.vertices()[i]));

        ymax = std::max(ymax,float(l.vertices()[i+1]));
        ymin = std::min(ymin,float(l.vertices()[i+1]));

        zmax = std::max(zmax,float(l.vertices()[i+2]));
        zmin = std::min(zmin,float(l.vertices()[i+2]));
    }

    alturaL = (ymax-ymin); //altura patricio

    centreLego = glm::vec3((xmax+xmin)/2, ymin, (zmax+zmin)/2);
}
...
void MyGLWidget::changeEscala(int s) {
    makeCurrent();
...
void MyGLWidget::setColorFons(int r, int g, int b) {
    makeCurrent();
...
void MyGLWidget::setAmple(int x) {
    makeCurrent();
...
void MyGLWidget::setAltura(int y) {
    makeCurrent();
