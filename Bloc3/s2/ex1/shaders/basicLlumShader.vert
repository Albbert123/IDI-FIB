...
vec3 Difus (vec3 NormSCO, vec3 L, vec3 colFocus)  //L = Focus - P
{
...
void main()
{	
    vec4 posFocus2 = vec4(posFocus,1.0);  //focus de camera
    //vec4 posFocus2 = view * vec4(posFocus,1.0);  //focus de escena
    vec4 P = view * TG * vec4 (vertex, 1.0);
    vec4 L = normalize(posFocus2-P);

    mat3 NormalMatrix = inverse (transpose (mat3 (view * TG)));
    vec3 normalsco = normalize(NormalMatrix*normal);

    vec3 difus = Difus(normalsco,L.xyz,colorFocus);
    vec3 ambient = Ambient();
    vec3 Lambert = ambient + difus;

    vec3 Phong = Especular(normalsco,L.xyz,P,colorFocus);
